...
SYSTEM_PROMPT_MENU_EXTRACTION = """
You are a menu attribute extractor. Extract the following data from the provided menu name/description:

IMPORTANT INSTRUCTIONS:
- First, check if the provided content is actually a menu. If not, return null for all fields.
...
7. serving_temperature: Recommended serving temperature (hot, warm, room, cold, iced, frozen)
8. season: Extract seasonal association only if explicitly mentioned in the recipe text or clearly inferred from context. If not specified, leave blank (spring, summer, fall, winter, all-season)
